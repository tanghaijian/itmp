(function(a, c) {
	var b = function(e, d) {
			...
			this.$element.on("expandAll", ".tree-folder-header", a.proxy(function(f) {
				this.expandAll(f.currentTarget)
			}, this));
			this.render()
		};
	b.prototype = {
			...
			this.options.dataSource.data(f.data(), function(g) {
				...
				a.each(g.data, function(h, j) {
					...
					if (j.type === "folder") {
						...
						if (typeof e.options['expandAll'] != "undefined" && e.options['expandAll'] == true) 
							setTimeout(function() {k.trigger('expandAll')}, 0);
					} else {
						if (j.type === "item") {
		...
		selectItem: function(e) {
			this.$element.find(".tree-active").removeClass("tree-active");
		...
		selectFolder: function(e) {
			...
			var f = d.parent();
			this.$element.find(".tree-active").removeClass("tree-active");
			d.find("div.tree-folder-name").addClass("tree-active");
			if (d.find("." + this.options["close-icon"]).length) {
				...
				} else if(typeof this.options['parentRel'] != "undefined" && this.options['parentRel'] == true){
						this.populate(d,1);
				} else  this.populate(d);
		...
		selectedItemsAndParents: function() {
			...
			　　　　　　var $parent = $(value).parent().prev("div.tree-folder-header");
