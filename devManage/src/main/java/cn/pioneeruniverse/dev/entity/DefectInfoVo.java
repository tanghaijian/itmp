...
public class DefectInfoVo extends BaseEntity {
    ...
    @JsonSerialize(using = ToStringSerializer.class)
    private Long testSetCaseExecuteId; //测试集案例执行表ID
    ...
    public void setDefectStatusList(String defectStatusList) {
        ...
        if(defectStatusList != null  && !defectStatusList.equals("")){
            ...
            this.defectStatusForStr = new Integer[str.length];
            for (int i = 0,len = str.length;i < len; i++){
                this.defectStatusForStr[i] = Integer.parseInt(str[i]);
            }
        }
    }
    ...
    public String getAssignUserIdStr() {
        return assignUserIdStr;
    }

    public void setAssignUserIdStr(String assignUserIdStr) {
        this.assignUserIdStr = assignUserIdStr;
        if(assignUserIdStr != null && !assignUserIdStr.equals("")){
            String[] str = assignUserIdStr.split(",");
            this.assignUserIdList = (Long[]) ConvertUtils.convert(str,Long.class);
        }
    }

    public Long[] getAssignUserIdList() {
        return assignUserIdList;
    }

    public void setAssignUserIdList(Long[] assignUserIdList) {
