		...
		$("#archivedCaseList").jqGrid('setGridParam', {
			...
			loadComplete: function (res) {
				if(res&&treeNode){ //数量对不上时 修改
					if(treeNode.archivedCaseCount!==res.records){
						var zTree = $.fn.zTree.getZTreeObj("menuTree");
						if(zTree){
							treeNode.archivedCaseCount=res.records
							zTree.updateNode( treeNode );
							$(".ztree li a.curSelectedNode>.tree_case_num").text("案例数:"+treeNode.archivedCaseCount)
						}
					}
...
function addHoverDom ( treeId, treeNode ){
	...
	if( treeNode.level == 0 ){
		...
		addStr = "<span class='tree_case_num'>案例数:"+ treeNode.archivedCaseCount +"</span><span class='button add' id='addBtn_" + treeNode.tId
		+ "' onfocus='this.blur();'></span>";
	}  
	sObj.after(addStr);
