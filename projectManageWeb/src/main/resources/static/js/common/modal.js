/**
 * Description: 各种弹框的js singleValue true 单选 false多选
 * Author:liushan
 * Date: 2018/12/24 下午 2:04
 */
var modalUrl = "/testManage/";
    ...
    $("#userTable").bootstrapTable({
        ...
        queryParams : function(params) {
            var param = {
                ...
                systemId:systemId,
                userPost:$("#userPost").val() != null?$("#userPost").val().toString():'',
                projectIds:$("#project").val() != null?$("#project").val().toString():""
            };
            return param;
        },
        columns : [{
...
function getProject(){
    $("#project").empty();
    $.ajax({
        url:modalUrl+"modal/getAllProject",
        dataType:"json",
        type:"post",
        async:false,
...
function getProjectBySystemId(systemId){
	$("#project").empty();
    $.ajax({
        url:modalUrl+"modal/getProject",
        dataType:"json",
        type:"post",
        data:{
        	systemId:systemId,
            ...
            if(data.status == 2){
            	...
            	if(data.project != null){
            		$.each(data.project,function(index,value){
            			$("#project").append("<option selected value='"+value.id+"'>"+value.projectName+"</option>");
            		})
            	}
                $('.selectpicker').selectpicker('refresh');
            }
...
function systemTableShow(singleValue){
    ...
    $("#systemTable").bootstrapTable({
        ...
        queryParams : function(params) {
            ...
            var projectId=$('#systemModal').attr('data-projectId')
            projectId?param.projectId=projectId:''
            return param;
        },
        columns : [{
            checkbox : true,
...
function reqTableShow(singleValue){
    ...
    $("#listReq").bootstrapTable({
        url:modalUrl+"modal/getAllRequirement",
        method:"post",
        contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
        queryParamsType:"",
        pagination : true,
    ...
    $("#testTaskTable").bootstrapTable({
        ...
        queryParams : function(params) {
            var param = {
                ...
                workTaskStatus:$("#taskState").val() != null?$("#taskState").val().toString():'',
...
function ComWindowShow(singleValue){
    ...
    if( $("#timeDate").val() != '' ){
	    ...
	    windowStartTime = $.trim(arr[0]);
	    windowEndTime =  $.trim(arr[1]);
    }  
...
/*-----------------测试任务弹框--------------*/
function DevPopup(singleValue){
    singleValue = singleValue == undefined?$("#TestPopupTable").bootstrapTable('getOptions').singleSelect:singleValue;
