...
$(() => {
  ...
  downOrUpButton();
  buttonClear();
  TimeConfig('#submitDate');
  TimeConfig('#approveDate');
})
...
function pageInit() {
  ...
  $("#project_list").jqGrid({
    url: $('#project_type').val() == 1 ? '/projectManage/change/getChanges' : '/projectManage/change/getChangesByProgram',
    datatype: 'json',
    mtype: "POST",
    height: 'auto',
    ...
    colNames: ['id', '序号', '变更事项', '状态', '变更提出日期', '变更批准日期', '分析汇总', '责任人', '执行结果说明', '操作'],
    colModel: [
      { name: 'id', hidden: true },
      { name: 'number', sortable: false },
      { name: 'changeItem', sortable: false },
      { name: 'changeStatusName', sortable: false },
      { name: 'submitDate' , sortable: false},
      { name: 'approveDate', sortable: false },
      { name: 'analyseSummary', sortable: false },
      { name: 'responsibleUser' , sortable: false},
      { name: 'resultExplain', width: 150, sortable: false },
      {
...
/**
 * @description type 空：新建,2：编辑,3：查看
 * @param id
 */
function addProject(type, ID) {
  ...
  if (type == 2 || type == 3) {
    $.ajax({
      type: "post",
      url: "/projectManage/change/getProjectChangeById",
      dataType: "json",
      data: {
        id: ID
      },
        ...
        if (result.status == 1) {
          ...
          if (type == 3) {
            ...
            logs.length && logs.map(function (val, idx) {
              let _className = val.logType == '新增变更' ?  '' : 'logDiv_contRemark';
              $('#logs_body').append(`
                  <div class="logDiv">
...
function add_submit() {
  ...
  $.ajax({
    type: "post",
    url: $("#edit_id").val() ? "/projectManage/change/updateProjectChange" : "/projectManage/change/insertProjectChange",
    dataType: "json",
...
function _delete(ID) {
  layer.confirm('确定要删除吗?', {
    ...
    $.ajax({
      type: "post",
      url: "/projectManage/change/deleteProjectChange",
      dataType: "json",
      data: {
        id: ID
      },
...
function formValidator() {
  let form_arr = ['changeItem', 'submitDate', 'changeStatus'];
...
function TimeConfig(ele,startDate){
  $(ele).datetimepicker({
    ...
    if(e.currentTarget.id == 'submitDate'){
      $('#approveDate').val('').change();
      $('#approveDate').datetimepicker('remove');
      TimeConfig('#approveDate',e.target.value);
    }
