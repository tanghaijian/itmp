 ...
 */
	...
	var collection_obj = {
	  menuUrl : '../report/toDevWorkReport',
	  filterName : '',
	  search : [],
	  isCollect : false
	};
	var iframeUrl = '/report/ureport/preview?_u=s3:';//ureport2预览统一url
	var report_1 = 'task_1.ureport.xml&_i=1&_r=1';//开发任务状态饼图预览参数
	var report_1b = 'developmentTasksList_1.ureport.xml&_i=1&_r=1';//开发任务状态table预览参数
	var report_2 = 'task_2.ureport.xml&_i=1&_r=1';//开发任务数量table预览参数
	...
	function other() {
	  ...
	  $('#defectStatus_table1').on('change', function () {
	    ...
	    var system = $('#system_table1').val() && $('#system_table1').val().join(',');
	    change_report({systemId:system,defectStatus:status,order:2});
	  })
	  $('#system_table1').on('change', function () {
	    if (!iframe_flag) return;
	    var status = $('#defectStatus_table1').val() && $('#defectStatus_table1').val().join(',');
	    var system = $('#system_table1').val() && $('#system_table1').val().join(',');
	    change_report({systemId:system,defectStatus:status,order:2});
