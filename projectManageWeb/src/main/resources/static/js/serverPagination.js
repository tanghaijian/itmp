...
(function($){
	...
	function initPageObj($this){
		var  pageSizeObj =  $this.find("select[name='pageSize']");
		var  pageIndexObj = $this.find("span[name='pageIndex']");
		var  pageFirstObj = $this.find("a[name='page_first']");
		var  pagePrevObj = $this.find("a[name='page_prev']");
		var  pageNextObj = $this.find("a[name='page_next']");
		var  pageLastObj = $this.find("a[name='page_last']");
		var  totalPageObj = $this.find("span[name='totalPage']");
		var  pageJumpNumObj = $this.find("input[name='pageJumpNum']");
		var  pageGoObj = $this.find("a[name='ucStatusFLGo']");
		var  startIndexObj = $this.find("span[name='startIndex']");
		var  endIndexObj = $this.find("span[name='endIndex']");
		var  totalRowNumObj = $this.find("span[name='totalRowNum']");
		
		pageObj.pageSizeObj = pageSizeObj;
		pageObj.pageIndexObj =pageIndexObj;
		pageObj.pageFirstObj = pageFirstObj;
		pageObj.pagePrevObj = pagePrevObj;
		pageObj.pageNextObj = pageNextObj;
		pageObj.pageLastObj = pageLastObj;
		pageObj.pageJumpNumObj = pageJumpNumObj;
		pageObj.totalPageObj = totalPageObj;
		pageObj.pageGoObj = pageGoObj;
		pageObj.startIndexObj = startIndexObj;
		pageObj.endIndexObj = endIndexObj;
		pageObj.totalRowNumObj = totalRowNumObj;
...
function initPage(pageInfo){
		...
		if (totalPage <= 1) {
			pageObj.pageFirstObj.parent().addClass("disabled");
			pageObj.pagePrevObj.parent().addClass("disabled");
			pageObj.pageNextObj.parent().addClass("disabled");
			pageObj.pageLastObj.parent().addClass("disabled");
		} else if (pageNum == 1) {
			pageObj.pageFirstObj.parent().addClass("disabled");
			pageObj.pagePrevObj.parent().addClass("disabled");
			pageObj.pageNextObj.parent().removeClass("disabled");
			pageObj.pageLastObj.parent().removeClass("disabled");
		} else if (pageNum < totalPage) {
			pageObj.pageFirstObj.parent().removeClass("disabled");
			pageObj.pagePrevObj.parent().removeClass("disabled");
			pageObj.pageNextObj.parent().removeClass("disabled");
			pageObj.pageLastObj.parent().removeClass("disabled");
		} else if (pageNum == totalPage) {
			pageObj.pageFirstObj.parent().removeClass("disabled");
